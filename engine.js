        ...
        card.innerHTML = `
            ...
            <div class="product-items">
                ${top3.map(match => {
                    ...
                    // Use Hugging Face dataset image
                    const imageUrl = prod.imageurl || '';
                    const price = prod.price_eur ? `${prod.price_eur} €` : 'Prix sur demande';
                    const matchTags = match.matchReasons.join(', ');
                    const category = [prod.category1_code, prod.category2_code, prod.category3_code].filter(Boolean).join(' › ');
                    ...
                    return `
                        <div class="product-item">
                            ...
                            <div class="product-item-info">
                                <div class="product-item-name">${prod.title || 'Produit Louis Vuitton'}</div>
                                <div class="product-item-desc">${category}</div>
                                ...
                                ${prod.itemurl ? `<a href="${prod.itemurl}" target="_blank" style="font-size:.7rem;color:#d4af37;margin-top:4px;display:inline-block">Voir sur LV →</a>` : ''}
                            </div>
                        </div>
                    `;
                }).join('')}
